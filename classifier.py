...
#                     print('Processing images for:', person_name)

#                     # Get image paths and labels for the current person
#                     image_paths, labels = facenet.get_image_paths_and_labels([person])  # Pass person as a list
#                     if len(image_paths) == 0:
#                         print('No images found for:', person_name)
#                         continue
...
#                     # Extract features (embeddings) for images of the current person
#                     emb_array = []
#                     image_size = 160
...
#                         emb_array.append(emb_batch)

#                     # Concatenate embeddings for all images of the current person
#                     embeddings = np.concatenate(emb_array, axis=0)

#                     # Save the embeddings for the current person
#                     classifier_file_name = os.path.expanduser(self.classifier_filename)
#                     with open(classifier_file_name, 'wb') as outfile:
#                         pickle.dump(embeddings, outfile)
#                     print('Embeddings saved for:', person_name)
